...
| Sofar Solar HYD 3…6K-EP (single phase, hybrid) | Modbus RTU over RS485 | **Experimental, but two-sourced** — sixteen of its eighteen registers agreed by the vendor protocol document and an independent configuration published as working on a real HYD-3600-EP; the two energy counters are vendor-only. The grid-power register is deliberately **not** mapped: no source states its sign. See [docs/sofar-hyd-protocol.md](docs/sofar-hyd-protocol.md) |
