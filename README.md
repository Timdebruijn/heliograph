...
| Sofar Solar HYD 3…6K-EP (single phase, hybrid) | Modbus RTU over RS485 | **Experimental, but two-sourced** — every mapped register agreed by the vendor protocol document and an independent configuration published as working on a real HYD-3600-EP. The grid-power register is deliberately **not** mapped: no source states its sign. See [docs/sofar-hyd-protocol.md](docs/sofar-hyd-protocol.md) |
