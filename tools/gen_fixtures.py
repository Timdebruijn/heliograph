...
def response(
    ctrl: int, func: int, payload: bytes, src_low: int = INVERTER_ADDR
) -> bytes:
...
INVERTER_ID = (
    b"1  3000E1.00   TL3000-20    Ever-Solar      XH300060115506193600V610-01023-06"
)
...
FIXTURES: list[tuple[str, str, bytes]] = [
    ...
    (
        "kRespNormalInfoCaptured",
        "Response: QUERY_NORMAL_INFO captured from a real TL3000-20"
    ...
    (
        "kRespNormalInfoDualExtended",
        "Response: QUERY_NORMAL_INFO, dual-generation 50-byte"
...
FIXTURES += [
    ...
    ("kRespPartial", "Partial: first 12 bytes of a valid response", _normal[:12]),
]
...
def main() -> None:
    ...
    out.append(
        f"inline constexpr double kEnergyTotalKwhReferenceBug = "
        f"{(E_TOTAL_LO / 10) + (E_TOTAL_HI * 65535 / 10):.1f};"
    )
    out.append(
        f"inline constexpr double kEnergyTotalExpectedDelta = {E_TOTAL_HI * 0.1:.1f};"
    )
    out.append("}  // namespace expected")
    out.append("")
    out.append(
        'inline constexpr const char* kExpectedSerial = "' + SERIAL.decode() + '";'
    )
    out.append(
        'inline constexpr const char* kExpectedInverterId = "'
        + INVERTER_ID.decode().rstrip()
        + '";'
    )
    out.append(
        'inline constexpr const char* kExpectedModel = "' + EXPECTED_MODEL + '";'
    )
    out.append("")
