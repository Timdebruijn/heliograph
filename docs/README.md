...
| [sofar-hyd-protocol.md](sofar-hyd-protocol.md) | Sofar HYD, both the **3…6K-EP** single-phase and the **5…20K-3PH** three-phase profiles. Better sourced than most — two independent sources agree on sixteen of the EP's eighteen registers, one of them running on real hardware; the three-phase profile's own L2/L3 rows are vendor-document only. The grid register is left out of both because nobody states its sign. |
